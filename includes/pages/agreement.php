...
function showAgreement($agreement=''){
		...
		echo renderTabs($nr, '', $tab_id_prefix, _AGREEMENT_OBJ, $data, $agreement->agreement_id, TRUE,
				renderAgreement(_AGREEMENT_OBJ, $agreement, null, "${tab_id_prefix}1", false), 1, _AGREEMENT_OBJ);
	    ?>
...
function showFinalisation($agreement=''){
		...
		//[number of tabs, label start, tab id start, type, data, id, render targets, active target content, active tab]
		echo renderTabs($nr, '', $tab_id_prefix, _AGREEMENT_OBJ, $data, $agreement->agreement_id, TRUE,
				renderFinalisation(_AGREEMENT_OBJ, $agreement, null, "${tab_id_prefix}1", false), 1, _AGREEMENT_OBJ);
	    ?>
	    <script type="text/javascript">
			activatetabs('tab_', [<?php echo implode(',', $tabs);?>]);
		</script>
